...
def QL_TD_error(beta, gamma, A_list, R_list, F_Pi_list, btsWts, state_action_features_list, next_state_action_features_list):
  beta = beta.reshape(A_list[0].shape[1], F_Pi_list[0].shape[1])
  TD = np.zeros(0) 
  for t in range(len(A_list)):
    if A_list[t].shape[0] > 2:
      F_Pi = F_Pi_list[t] 
      R = R_list[t][:-1]
      state_action_features = state_action_features_list[t]    
      state_action_features_next = next_state_action_features_list[t]
      #Qmax = np.max(np.dot(F_Pi[1:,:], beta.T), axis=1)
      Qnext = np.dot(state_action_features_next, beta.ravel()) #SARSA!
      Q = np.dot(state_action_features, beta.ravel())
      #TD_t = R + gamma*Qmax - Q 
      TD_t = R + gamma*Qnext - Q
      if btsWts is not None:
        TD_t = np.multiply(TD_t, btsWts[t][:-1])
      TD = np.append(TD, TD_t)
  return TD

def QL_objective(beta, gamma, A_list, R_list, F_Pi_list, btsWts, state_action_features_list, next_state_action_features_list):
  TD = QL_TD_error(beta, gamma, A_list, R_list, F_Pi_list, btsWts, state_action_features_list, next_state_action_features_list)
  return np.dot(TD, TD)
...
def betaOptQL(gamma, A_list, R_list, F_Pi_list, wStart=None, bts=True):
  ...
  if totalSteps < nV: 
    ...
    if bts: 
      ...
      btsWts = None
    state_action_features_list = [np.array([np.kron(A_list[t][i,:], F_Pi_list[t][i,:]) for i in range(A_list[t].shape[0]-1)]) for t in range(len(A_list))]
    next_state_action_features_list = [np.array([np.kron(A_list[t][i,:], F_Pi_list[t][i,:]) for i in range(1, A_list[t].shape[0])]) for t in range(len(A_list))]
    objective = lambda beta: QL_objective(beta, gamma, A_list, R_list, F_Pi_list, btsWts, state_action_features_list, next_state_action_features_list)
    if wStart is None:       
